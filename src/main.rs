...
fn load_game(save_folder: &str) {
    ...
    player.skills = character_data
        ...
        .map(|(skill_name, level, experience)| {
            ...
            ) // Placeholder for loading skills
        })
        .collect();
...
fn save_game(player: &Player, game_map: &Map, save_folder: &str, character_name: &str) {
    // Create character save file using serialized player data
    ...
    // Create map save file using serialized map data
...
fn game_loop(
    ...
    loop {
        ...
        let view_size = if let Some((_, height)) = term_size::dimensions() {
            if (height >= 44) {
                // Considering the entire UI (menu, map, actions)
                30 // 30x30 chunk
            } else if (height >= 29) {
                20 // 20x20 chunk
            } else {
        ...
        match input {
            ...
            "i" => {
                new_action = display_and_handle_inventory(&mut player, None);
            }
            input if input.starts_with("eat ") => {
                let item_name = input.trim_start_matches("eat ").trim();
                new_action = inventory::handle_eat_command(&mut player, item_name);
            }
            _ => {
